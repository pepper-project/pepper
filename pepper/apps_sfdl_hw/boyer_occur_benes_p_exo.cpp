#include <apps_sfdl_hw/boyer_occur_benes_p_exo.h>
...
boyer_occur_benesProverExo::boyer_occur_benesProverExo() {
...
void boyer_occur_benesProverExo::init_exo_inputs(
  const mpq_t* input_q, int num_inputs,
  char *folder_path, HashBlockStore *bs) {
  
}

void boyer_occur_benesProverExo::export_exo_inputs(
  const mpq_t* output_q, int num_outputs,
  char* folder_path, HashBlockStore *bs) {

}

void boyer_occur_benesProverExo::run_shuffle_phase(char *folder_path) {

}

void boyer_occur_benesProverExo::baseline_minimal(void* input, void* output){
  //Run the computation
}

using boyer_occur_benes_cons::ALPHABET_LENGTH;
using boyer_occur_benes_cons::PATTERN_LENGTH;
void boyer_occur_benesProverExo::baseline(const mpq_t* input_q, int num_inputs, 
      mpq_t* output_recomputed, int num_outputs) {

    unsigned input[PATTERN_LENGTH];
    unsigned output[ALPHABET_LENGTH];
    ...
    for(unsigned i=0; i < PATTERN_LENGTH - 1; i++) {
        unsigned addr = input[i];
...
//Refer to apps_sfdl_gen/boyer_occur_benes_cons.h for constants to use in this exogenous
//check.
bool boyer_occur_benesProverExo::exogenous_check(const mpz_t* input, const mpq_t* input_q,
