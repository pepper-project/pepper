#include <apps_sfdl_hw/cql_ro_threshold_p_exo.h>
...
cql_ro_thresholdProverExo::cql_ro_thresholdProverExo() { }

void cql_ro_thresholdProverExo::init_exo_inputs(
  const mpq_t* input_q, int num_inputs,
  char *folder_path, HashBlockStore *bs) {
}

void cql_ro_thresholdProverExo::export_exo_inputs(
  const mpq_t* output_q, int num_outputs,
  char* folder_path, HashBlockStore *bs) {
}

void cql_ro_thresholdProverExo::run_shuffle_phase(char *folder_path) {
...
int compute(struct In *input, struct Out *output) {
  ...
  //CQL("SELECT KEY, FName, LName, Age, Major, State, PhoneNum, Class, Credits, Average, Honored FROM Student WHERE Average >= 90 LIMIT 5", output->result);
  {
    ...
    tree_find_gt(&(Average_index), (90), 1, &(tempResult));
    for (i = 0; i < 5; i++) {
...
void cql_ro_thresholdProverExo::baseline(const mpq_t* input_q, int num_inputs, 
...
//Refer to apps_sfdl_gen/cql_ro_threshold_cons.h for constants to use in this exogenous
//check.
bool cql_ro_thresholdProverExo::exogenous_check(const mpz_t* input, const mpq_t* input_q,
